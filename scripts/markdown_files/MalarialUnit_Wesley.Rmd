...
```{r}
gstat <- (filter(malPop, Year == 2022) %>% transmute(Observed/Pop))$`Observed/Pop`%>% 
  localG(mat2listw(mat1, style = 'B'), return_internals = F)



temp <- st_as_sf(filter(malPop, Year == 2022) %>% inner_join(boundaries, by = join_by(Name == admin1Name)))
