...
Fit GLM (I hope). This is difficult since there is this constant value within.
```{r}
GLM1 <- lm(log(round(malPop$Observed))-log(round(malPop$Expected)) ~ malPop$Pop)

summary(GLM1)

GLM1.5 <- glm(round(malPop$Observed) ~   malPop$Pop)

summary(GLM1.5)

GLM2 <- glm(round(malPop$Observed) ~ 1*log(round(malPop$Expected)) + malPop$Pop,
            family = poisson)
